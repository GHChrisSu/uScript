...
public class Follow : MonoBehaviour
{
	...
	void Start()
	{
		...
		tween = transform.DOMove(target.position, 0.5f).SetAutoKill(false);
        transform.DORotate(new Vector3(0, 0, 50), 0.1f).SetLoops(-1, LoopType.Incremental);
	...
	void Update()
	{
        ...
        if (targetLastPos == target.position - offset) return;
		// Add a Restart in the end, so that if the tween was completed it will play again
		tween.ChangeEndValue(target.position - offset, true).Restart();
